                ...
                <div class="card" style="border: 2px solid rgba(13, 148, 136, 0.15); background: rgba(255, 255, 255, 0.85); backdrop-filter: blur(12px); box-shadow: 0 12px 40px rgba(0, 0, 0, 0.08); border-radius: 1.25rem; overflow: hidden;">
                    ...
                    <form method="POST" action="{{ route('listings.update', $listing) }}" enctype="multipart/form-data">
                        ...
                        <div style="margin-bottom: 2.5rem;">
                            ...
                            <div class="row g-3">
                                @php
                                    $listingCategoryIds = old('device_type_ids', $listing->deviceTypes->pluck('id')->toArray());
                                    if (empty($listingCategoryIds) && $listing->device_type_id) {
                                        $listingCategoryIds = [$listing->device_type_id];
                                    }
                                @endphp
                                ...
                                <div class="col-md-6" id="bulkCountWrapper" style="display: none;">
                                    ...
                                    <input type="number" min="2" max="1000" class="form-control @error('lot_item_count') is-invalid @enderror" 
                                           id="lot_item_count" name="lot_item_count" value="{{ old('lot_item_count', $listing->lot_item_count ?? 4) }}"
                                ...
                                <div class="col-12" id="bulkCategoryWrapper" style="display: none;">
                                    ...
                                    <div class="row g-2" id="bulkCategoryGrid">
                                        @foreach($deviceTypes as $type)
                                            @php
                                                $isChecked = in_array($type->id, $listingCategoryIds);
                                            @endphp
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', function () {
        ...
        const listingForm = document.getElementById('listingForm') || document.querySelector('form[action*="listings"]');
        if (listingForm) {
